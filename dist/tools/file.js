...
export function markFileAsRead(path) {
...
export function hasFileBeenRead(path) {
    const fullPath = resolve(path);
...
export function getFileModifiedExternally(path) {
    const fullPath = resolve(path);
    const cached = readFileCache.get(fullPath);
    if (!cached)
        return false;
...
export const editFileTool = {
    definition: {
        ...
        function: {
            name: 'edit_file',
            description: 'Edit a file by replacing a specific string with new content. The old_string must match exactly what is in the file (WITHOUT line numbers - line numbers shown by read_file are for display only). The old_string must be unique in the file.',
            parameters: {
                type: 'object',
                properties: {
                    path: {
