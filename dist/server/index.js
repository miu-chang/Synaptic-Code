 ...
 */
function updateUsage(key, tokens) {
    const keys = loadApiKeys();
    const found = keys.find(k => k.key === key);
    if (found) {
...
export function revokeApiKey(keyPreview) {
    const keys = loadApiKeys();
    const index = keys.findIndex(k => k.key.slice(0, 12) + '...' + k.key.slice(-4) === keyPreview ||
        k.key === keyPreview);
    if (index >= 0) {
        keys.splice(index, 1);
        saveApiKeys(keys);
 ...
 */
async function proxyToLmStudio(req, res, lmStudioUrl, apiKey) {
    const url = new URL(req.url || '/', `http://${req.headers.host}`);
