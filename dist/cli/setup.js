...
import { getLicenseStatus, activateLicense, startTrial, hasValidAccess, isValidKeyFormat, maskLicenseKey, } from '../license/index.js';
...
function getTranslatedOptionalModels() {
    const desc = t().setup.modelDesc;
 ...
 */
function isLmStudioAppInstalled() {
    if (DEBUG_FORCE_NOT_INSTALLED)
        return false;
    const os = platform();
    if (os === 'darwin') {
