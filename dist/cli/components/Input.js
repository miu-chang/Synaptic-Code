...
function AnimatedDots({ startedAt }) {
    ...
    return _jsxs(Text, { dimColor: true, children: ["Processing", dotsStr, timeStr] });
}
...
export function Input({ onSubmit, placeholder = 'Type a message...', loading = false, loadingStartedAt }) {
    ...
    return (_jsxs(Box, { flexDirection: "column", children: [showCommandHints && (_jsxs(Box, { flexDirection: "column", marginBottom: 1, paddingX: 1, children: [matchingCommands.map((cmd, i) => (_jsxs(Box, { children: [_jsx(Text, { color: i === commandIndex ? 'cyan' : 'gray', children: i === commandIndex ? '❯ ' : '  ' }), _jsxs(Text, { color: i === commandIndex ? 'cyan' : undefined, bold: i === commandIndex, children: ["/", cmd.name] }), cmd.shortcut && (_jsxs(Text, { dimColor: true, children: [" [", cmd.shortcut, "]"] })), _jsxs(Text, { dimColor: true, children: [" - ", cmd.description] })] }, cmd.name))), _jsx(Text, { dimColor: true, children: "  \u2191\u2193 select \u2022 Tab complete \u2022 Enter execute" })] })), _jsxs(Box, { borderStyle: "round", borderColor: loading ? 'gray' : isTruncated ? 'yellow' : 'cyan', paddingX: 1, children: [_jsx(Text, { color: "cyan", children: "\u276F " }), loading ? (_jsx(AnimatedDots, { startedAt: loadingStartedAt })) : isTruncated ? (
                    // Show truncated preview for long/multiline input
                    _jsxs(Box, { flexDirection: "column", children: [_jsx(Text, { color: "yellow", children: displayValue }), _jsx(Text, { dimColor: true, children: "Press Enter to send, Esc to clear" })] })) : (_jsx(TextInput, { value: value, onChange: handleChange, onSubmit: handleSubmit, placeholder: placeholder }))] })] }));
}
