 ...
 */
function convertToJpeg(inputPath: string): Buffer | null {
 ...
 */
export function loadImageFromFile(filePath: string): ImageData | null {
  try {
 ...
 */
export function getClipboardImage(): ImageData | null {
  if (process.platform !== 'darwin') {
  ...
  try {
    ...
    try {
      const result = execSync(
        `osascript -e 'the clipboard as «class PNGf»' 2>/dev/null | xxd -r -p`,
