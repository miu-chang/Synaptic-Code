...
import { setTodoManager, getTodoManager } from '../tools/todo.js';
import type { Settings, ProviderType } from '../config/settings.js';
import {
  getProviderModel,
  setProviderModel,
  getProviderBaseUrl,
...
import { selectOption, selectProvider, type SelectorOption } from './selector.js';
      ...
      case 'history':
        ...
        if (convs.length === 0) {
          ...
          convs.slice(0, 10).forEach((c, i) => {
            console.log(
              `${chalk.dim(`${i + 1}.`)} ${c.title} ${chalk.dim(`(${new Date(c.updatedAt).toLocaleDateString()})`)}`
            );
          });
          console.log();
        }
...
`);
  ...
  private async selectModel(): Promise<void> {
    ...
    try {
      ...
      const options: SelectorOption[] = models.map((m) => ({
        label: m,
        value: m,
      }));
  ...
  private async selectProvider(): Promise<void> {
    ...
    if (selected) {
      this.settings.provider = selected as ProviderType;
      const { baseUrlOrApiKey, model } = getClientArgs(this.settings);
      this.client = createClient(this.settings.provider, baseUrlOrApiKey, model);
