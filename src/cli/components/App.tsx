...
import { PlanView, type PlanItem } from './PlanView.js';
...
import type { LLMClient, Message, MessageContent } from '../../llm/types.js';
 ...
 */
function setTabActivity(active: boolean): void {
  if (active) {
...
interface DisplayMessage {
  ...
  content: string;
  toolName?: string;
  isExecuting?: boolean;  // Tool is currently running
...
export function App({ settings: initialSettings, client: initialClient, tools, licenseStatus, isGitRepo: initialIsGitRepo, synapticStatus: initialSynapticStatus, initialMessages }: AppProps): React.ReactElement {
  ...
  const [compactingStartedAt, setCompactingStartedAt] = useState<number | null>(null);
  const [messages, setMessages] = useState<DisplayMessage[]>(
    () => initialMessages?.map((m, i) => ({ ...m, key: i })) ?? []
  );
  ...
  const [synapticStatus, setSynapticStatus] = useState<string>(initialSynapticStatus ?? '');
  const [todos, setTodos] = useState(todoManager.getAll());
  const [todoExpanded, setTodoExpanded] = useState(false);
  const [agentState, setAgentState] = useState<AgentState | null>(null);
  const agentRef = useRef<Agent | null>(null);
  const [subAgentStatuses, setSubAgentStatuses] = useState<Map<string, SubAgentStatus>>(new Map());
  const completionQueueRef = useRef<AgentCompletion[]>([]);
  const [pendingPlan, setPendingPlan] = useState<PlanStep[] | null>(null);
  const pendingToolCallsRef = useRef<any>(null);  // Store tool calls waiting for approval
  const pendingMessageRef = useRef<any>(null);    // Store message waiting for approval
  const [autoAccept, setAutoAccept] = useState(true);  // true = auto execute, false = require approval
  const [lmsDownload, setLmsDownload] = useState<LmsDownloadProgress | null>(null);
  ...
  useEffect(() => {
    if (initialIsGitRepo !== undefined) return; // Skip if already provided
    import('fs').then(fs => {
  ...
  useEffect(() => {
    ...
    const handleModelChange = (change: LmsModelChange) => {
  ...
  const executeToolCalls = useCallback(async (msg: Message) => {
  ...
  const continueAfterTools = useCallback(async (signal: AbortSignal): Promise<Message> => {
    setIsLoading(true);
  ...
  const handlePlanApproval = useCallback(async () => {
    if (!pendingToolCallsRef.current || !pendingMessageRef.current) return;
    ...
    try {
      ...
      if (!isCloudProvider(settings.provider)) {
        ...
        if (tokenCount > threshold && !conversationManager.isCompressed()) {
          ...
          try {
            const result = await conversationManager.compactWithLLM(
              client,
  ...
  const handleProviderSelect = (provider: ProviderType, apiKey?: string) => {
    const newSettings = { ...settings, provider };

    // Save API key if provided
    if (apiKey && isCloudProvider(provider)) {
      setCloudProviderApiKey(newSettings, provider as CloudProviderType, apiKey);
    }
  ...
  const handleRestore = (pointId: number, mode: 'fork-both' | 'undo-code' | 'fork-conversation' | 'cancel') => {
    if (mode === 'cancel') {
      setViewMode('chat');
      return;
    }
  ...
  const renderMessage = (msg: DisplayMessage, idx: number) => {
    ...
    return (
      <Box key={idx} marginY={0} marginTop={marginBefore ? 1 : 0} marginBottom={marginAfter ? 1 : 0} flexDirection="column">
        {msg.type === 'user' && (
          <Box>
      ...
      {msg.type === 'assistant' && msg.content && msg.content.trim() && (
        <Box flexDirection="column">
          ...
          <Box paddingLeft={2}>
            <Text wrap="wrap">{msg.content.trim()}</Text>
          </Box>
        </Box>
      )}
      {msg.type === 'tool' && (
      ...
      {msg.type === 'info' && (
        <Text color="cyan">ℹ {msg.content}</Text>
      )}
    </Box>
    );
  };
  ...
  return (
    ...
    <Box flexDirection="column" padding={1}>
      <Banner cwd={process.cwd()} isGitRepo={isGitRepo} licenseStatus={licenseStatus} />
      ...
      {/* Current streaming response - show last portion to reduce re-render */}
      {streamingText && (
        <Box flexDirection="column" marginY={1}>
          <Text bold color="green">◆ Assistant</Text>
          {scrollPaused && <Text dimColor> [paused]</Text>}
          <Box paddingLeft={4}>
            <Text wrap="wrap">
      ...
      {isCompacting && <CompactingIndicator startedAt={compactingStartedAt ?? undefined} />}
      {lmsDownload && (
        <DownloadIndicator
          model={lmsDownload.model}
          status={lmsDownload.status}
          progress={lmsDownload.progress}
          message={lmsDownload.message}
        />
      )}
      ...
      {viewMode === 'approval' && pendingPlan && (
        <PlanView
          items={pendingPlan}
          onApprove={() => handlePlanApproval()}
          onApproveAll={() => handlePlanApproval()}
      ...
      {viewMode === 'license' && (
        <LicenseView
          onClose={() => setViewMode('chat')}
          onMessage={(type, content) => addMessage({ type, content })}
        />
      )}
      ...
      <StatusBar
        ...
        tokenCount={conversationManager.getTokenCount()}
        maxTokens={conversationManager.getMaxTokens()}
        isCompressed={conversationManager.isCompressed()}
        lastUsage={lastUsage}
      ...
      <TodoBar
        ...
        onToggle={() => setTodoExpanded(prev => !prev)}
      />

      {/* Sub-agent status bar */}
      {subAgentStatuses.size > 0 && (
        <SubAgentBar statuses={subAgentStatuses} />
      )}

      {/* Synaptic status */}
      {synapticStatus && (
        <Text dimColor color="magenta">{synapticStatus}</Text>
      )}
    </Box>
