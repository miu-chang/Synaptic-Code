...
interface TrialInfo {
  ...
  days: number;
  machineHash: string;
  serverVerified?: boolean;  // True if server knows about this trial
}
 ...
 */
function saveLicense(license: StoredLicense): void {
  try {
    ensureLicenseDir();
...
function loadTrialInfo(): TrialInfo | null {
  try {
    ...
    return JSON.parse(data) as TrialInfo;
  } catch {
    return null;
  }
}
 ...
 */
function checkTrialStatusLocal(): { valid: boolean; daysLeft: number; started: number } | null {
 ...
 */
async function verifyTrialOnline(): Promise<{
  valid: boolean;
  daysLeft: number;
  started: number;
  error?: string;
 ...
 */
async function registerTrialOnline(): Promise<{
  success: boolean;
  daysLeft?: number;
  startedAt?: number;
  error?: string;
 ...
 */
export async function startTrial(): Promise<LicenseInfo> {
 ...
 */
export async function verifyTrial(): Promise<LicenseInfo> {
 ...
 */
async function verifyOnline(licenseKey: string): Promise<{
  valid: boolean;
  plan?: string;
  expiresAt?: number | null;
  error?: string;
 ...
 */
async function activateOnline(licenseKey: string, email?: string): Promise<{
  success: boolean;
  plan?: string;
  expiresAt?: number | null;
  error?: string;
  ...
  try {
    ...
    const data = await response.json() as { success?: boolean; plan?: string; expiresAt?: string; error?: string };
...
export async function activateLicense(
  key: string,
  email?: string
): Promise<{ success: boolean; info: LicenseInfo; message?: string }> {
