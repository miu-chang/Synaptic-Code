...
export interface LmsDownloadProgress {
  ...
  progress?: string; // e.g., "45%" or "1.2GB / 4.5GB"
  message?: string;
}
 ...
 */
export const lmsGetModel: ToolHandler = {
  definition: {
    type: 'function',
    function: {
      name: 'lms_get_model',
      description:
        'Download a model to LM Studio. Use this when the user asks to install, download, or get a model. The best matching model will be automatically selected and downloaded. Examples: "qwen3.5 9b", "llama 3.1 8b", "gemma 2b".',
      parameters: {
        type: 'object',
        properties: {
          query: {
            type: 'string',
            description:
              'Search query for the model. Be specific with model name and size (e.g., "qwen3.5 9b", "llama 3.1 70b", "gemma2 2b")',
          },
        },
        required: ['query'],
      },
 ...
 */
export const lmsListModels: ToolHandler = {
  definition: {
    type: 'function',
    function: {
      name: 'lms_list_models',
      description:
        'List all models installed in LM Studio. Shows model name, size, architecture, and whether it is currently loaded.',
      parameters: {
        type: 'object',
        properties: {},
        required: [],
 ...
 */
export const lmsLoadModel: ToolHandler = {
  definition: {
    type: 'function',
    function: {
      name: 'lms_load_model',
      description:
        'Load a model into memory in LM Studio. Can specify context length and GPU offload. Use this when user wants to change context size or reload a model with different settings.',
      parameters: {
        type: 'object',
        properties: {
          model: {
  ...
  execute: async (args) => {
    ...
    let contextLength = args.context_length as number | undefined;
    const gpu = args.gpu as string | undefined;
    ...
    // Parse GPU option
    let gpuOption: 'off' | 'max' | number | undefined;
    if (gpu === 'off' || gpu === 'max') {
      gpuOption = gpu;
    } else if (gpu) {
    ...
    if (result.success) {
      lmsEvents.emit('modelChange', { model, action: 'loaded' } as LmsModelChange);
    }
 ...
 */
export const lmsUnloadModel: ToolHandler = {
  definition: {
    type: 'function',
    function: {
 ...
 * All LM Studio tools
 */
export const lmsTools: ToolHandler[] = [
  lmsGetModel,
  lmsListModels,
  lmsLoadModel,
  lmsUnloadModel,
