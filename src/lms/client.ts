 ...
 */
export function isLmsInstalled(): boolean {
  try {
 ...
 */
export function listModels(): LmsModel[] {
  try {
 ...
 */
export function getLoadedModelsInfo(): LoadedModelInfo[] {
  try {
    const output = execSync(`${LMS_PATH} ps`, { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'] });
 ...
 */
function parseLoadedModelsInfo(output: string): LoadedModelInfo[] {
  const models: LoadedModelInfo[] = [];
  const lines = output.split('\n');

  for (const line of lines) {
    // Skip header and empty lines
    if (line.includes('IDENTIFIER') || !line.trim()) continue;
 ...
 */
export async function unloadModel(all = true): Promise<{ success: boolean; message: string }> {
  return new Promise((resolve) => {
 ...
 */
export function searchModels(query: string, _limit = 5): string[] {
 ...
 * Get full status
 */
export async function getStatus(): Promise<LmsStatus> {
 ...
 */
export async function ensureReady(defaultModel?: string, options?: LoadModelOptions): Promise<{
  success: boolean;
  message: string;
  actions: string[];
}> {
 ...
 */
export async function ensureContextLength(
  ...
  if (targetModel) {
    ...
    const found = loadedModels.find(m =>
      m.model.includes(targetModel) || targetModel.includes(m.model) ||
  ...
  const bestInstance = sameModelInstances.reduce((best, curr) =>
    curr.context > best.context ? curr : best
  , sameModelInstances[0]);
...
// Parser helpers
function parseModelList(output: string): LmsModel[] {
  const models: LmsModel[] = [];
  const lines = output.split('\n');
...
function parseLoadedModels(output: string): string[] {
  const models: string[] = [];
  const lines = output.split('\n');
