...
export interface ExecutionRecord {
  ...
  result?: unknown;
  error?: string;
  // Extracted key info for easy reference
  summary: string;
...
function generateSummary(
  ...
  if (server === 'unity') {
    ...
    if (tool.includes('scene_summary') || tool.includes('scene_info')) {
      const r = result as { scene_name?: string } | undefined;
 ...
 */
function extractModifiedObjects(
  server: 'blender' | 'unity',
  tool: string,
  params: Record<string, unknown>
): string[] | undefined {
 ...
 */
export function getRecentExecutions(count = 10): ExecutionRecord[] {
 ...
 * Get last execution
 */
export function getLastExecution(): ExecutionRecord | undefined {
  return history[0];
}
 ...
 */
export function getLastExecutionFor(server: 'blender' | 'unity'): ExecutionRecord | undefined {
 ...
 */
export function getLastCreatedObject(): { server: 'blender' | 'unity'; name: string } | undefined {
 ...
 */
export function findByObjectName(name: string): ExecutionRecord | undefined {
  return history.find(r =>
    r.createdObjects?.includes(name) ||
 ...
 */
export function getHistorySummary(count = 5): string {
